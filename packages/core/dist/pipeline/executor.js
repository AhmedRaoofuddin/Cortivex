...
export class PipelineExecutor extends EventEmitter {
    ...
    async execute(pipeline, options = {}) {
        const runId = uuidv4();
        const { failureStrategy = 'stop', maxRetries = 1, parallelism = 4, verbose = false, targetDir = process.cwd(), model, } = options;
            ...
            for (const batch of executionOrder) {
                ...
                for (const chunk of batchChunks) {
                    ...
                    const promises = chunk.map(async (node) => {
                        ...
                        await this.executeNode(node, run, {
                            runId,
                            targetDir,
        ...
        while (processed.size < nodes.length) {
            ...
            if (batch.length === 0) {
                ...
                const remaining = nodes
                    ...
                    .map((n) => n.id)
                    .join(', ');
                throw new Error(`Cannot resolve execution order: circular dependency detected among nodes: ${remaining}`);
            }
            batches.push(batch);
    ...
    evaluateCondition(condition, completedNodes, run) {
        ...
        const containsMatch = condition.match(/^([\w][\w-]*)\.output\.contains\(['"](.+)['"]\)$/);
        if (containsMatch) {
    ...
    getFilesForNode(nodeType) {
        if (!nodeType)
            return [];
        // Only claim files for nodes that can write
        const hasWriteTools = nodeType.tools.some((t) => ['Edit', 'Write', 'write_file'].includes(t));
        if (hasWriteTools) {
